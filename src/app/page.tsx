...
export default async function Home() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      {/* Section Heading */}
      <div className="flex flex-col sm:flex-row items-center justify-between mb-8 border-b-2 border-gray-300 pb-4">
        <h2 className="text-3xl font-semibold text-gray-800 text-center sm:text-left">
          Learn from Our Collection of Code Snippets
        </h2>
        <div className="mt-4 sm:mt-0 flex gap-4">
          <Link href="/snippet/new">
            <Button className="bg-blue-600 text-white hover:bg-blue-500 text-lg px-6 py-2 rounded-lg transition duration-300">
              Add New
            </Button>
          </Link>
          <Link href="/generate">
      ...
      {snippets && snippets.length > 0 ? (
        ...
        <div className="text-center text-gray-500 mt-6">No snippets available at the moment.</div>
      )}
    </div>
  );
}
